...
// A 403 (this model is off-limits to the key's tier) benches the model+key for
// a full day — it won't recover within a request's retry window. Upstream
// exported this from ratelimit; the fork has no such constant, so define it here
// (matches PAYMENT_REQUIRED_COOLDOWN_MS = one day).
const MODEL_FORBIDDEN_COOLDOWN_MS = 24 * 60 * 60 * 1000;
...
anthropicRouter.post('/messages', async (req: Request, res: Response) => {
  ...
  // The fork's sticky-model store is keyed by (apiKey, session) — thread the token.
  const apiKey = extractApiToken(req);
  let preferredModel = resolved.preferredModelDbId;
  if (preferredModel == null) preferredModel = getStickyModel(apiKey, messages, sessionId);
    ...
    try {
      if (stream) {
        await streamCompletion(res, route, messages, completionOptions, {
          ...
          sessionId, pinned: resolved.pinned, apiKey,
        });
        return;
      }
      ...
      if (!resolved.pinned) setStickyModel(apiKey, messages, route.modelDbId, sessionId);
...
anthropicRouter.get('/models', (req: Request, res: Response, next: NextFunction) => {
  ...
  // Same catalog the OpenAI /models endpoint lists: enabled models that are in
  // the fallback chain and have an enabled key, smartest-first. (Mirrors the
  // SELECT in routes/proxy.ts; the fork has no buildModelListing helper.)
  ...
  const data = [
    ...
    ...rows.map(m => ({ type: 'model' as const, id: `${m.platform}/${m.model_id}`, display_name: m.display_name, created_at: MODEL_CREATED_AT })),
  ];
