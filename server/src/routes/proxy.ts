...
function buildModelCapabilities(modelId: string, maxOutputTokens: number | null, supportsVision: boolean, supportsTools: boolean) {
  ...
  const reasoningFamily =
    ...
    || ml === 'coding-glm-5.2-free';                  // aihubmix glm 5.2 free — exact-id match
...
proxyRouter.post('/chat/completions', async (req: Request, res: Response) => {
  ...
  outerLoop: for (let totalAttempt = 0; ; totalAttempt++) {
    ...
    // MiniMax M2.x/M3 on tokenrouter / openrouter / nvidia returns reasoning
    ...
    keyRetry: for (let keyAttempt = 0; keyAttempt < PER_KEY_RETRIES; keyAttempt++) {
      ...
      // Refusals ARE charged: the provider saw the request and may have counted
      // it on its side, and undercounting silently overruns a real budget.
      // Whether the failure also escalates the BENCH onto the daily ladder is a
      // separate question — see indicatesQuotaExhaustion at the cooldown below.
    ...
    if (providerAtFault(lastError)) {
      setCooldown(
        ...
        computeRetryCooldownMs(
          ...
          // Escalate onto the daily ladder only when the failure actually says
          // the key is out of budget — not when our own request was wrong.
          indicatesQuotaExhaustion(lastError),
        ),
      );
    }
    recordRateLimitHit(route.modelDbId);
