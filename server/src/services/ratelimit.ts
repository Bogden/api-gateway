...
/** Move the request ledger (in-memory windows + persisted usage rows) for one
 *  attempt against this model+key. Shared by the success path and the
 *  reached-the-provider failure path — both consume the provider's request
 *  quota, so both must count. */
